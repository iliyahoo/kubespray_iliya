...
$kube_node_instances = $num_instances - $kube_master_instances
...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances).each do |i|
    if i <= $etcd_instances
      vm_memory = $vm_etcd_memory
    else
      vm_memory = $vm_node_memory
    end
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
     ...
     config.vm.provider :libvirt do |lv|
      ...
      # Provision your existing Public SSH key
      if $os == 'fedora'
        src_key = '~vagrant/sync'
      else
        src_key = '/vagrant'
      end
      config.vm.provision "shell" do |s|
        s.path = 'init_script.sh'
        s.args = [src_key]
      end
      ...
      # if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            ...
            "kube-node" => ["#{$instance_name_prefix}-0[#{$kube_master_instances + 1}:#{$num_instances}]"],
